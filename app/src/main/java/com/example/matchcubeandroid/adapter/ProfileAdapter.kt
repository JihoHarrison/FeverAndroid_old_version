...
class
ProfileAdapter (val context: Context, val profileLists: ArrayList<ProfileModel>) : RecyclerView.Adapter<ProfileAdapter.TeamViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ProfileAdapter.TeamViewHolder, position: Int) { //  실제로 호출 시 onBindViewHolder가 지속적으로 호출됨
        Glide.with(context).load(profileLists.get(position).imgTeams).into(holder.imgTeams)
